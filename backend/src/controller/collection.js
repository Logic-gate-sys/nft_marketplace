...
import { ipfsCIDToHttp, fetchIpfsMetadata, decodeOnChainTokenURI, } from '../utils/ifpfs';
// coverImage: null,
// logoImage: null,
// creacte collection
export const create_offchain_collection = async (req, res) => {
    // get fields require to create collection
    const { owner_id, type, contractAddress, name, description, symbol, category, royalties, } = req.body;
    ...
    //image uri
    const cover_uri = await uploadImageToPinata(cover.buffer, cover.originalname, cover.mimetype);
    ...
    const logo_uri = await uploadImageToPinata(logo.buffer, logo.originalname, logo.mimetype);
    if (!logo_uri) {
    ...
    const col_uri = await uploadCollectionMetaData(type, contractAddress, name, description, symbol, category, royalties, cover_uri, logo_uri);
    if (!col_uri) {
...
export const fetchAllCollections = async (req, res) => {
    try {
        // Query params
        const { category, sortBy = 'recent', page = '1', limit = '20', search, } = req.query;
        ...
        const transformedCollections = await Promise.all(collections.map(async (col) => {
            ...
            const transformedNfts = await Promise.all((col.Nft ?? []).map(async (nft) => {
                // OFF-CHAIN NFT
...
export const fetchCollectionById = async (req, res) => {
    try {
        ...
        const transformedNfts = await Promise.all((collection.Nft ?? []).map(async (nft) => {
            // OFF-CHAIN NFT
            if (meta.type === 'offchain') {
                if (!nft.uri) {
                    return;
 ...
 * @returns
 */
export const fetchCollectionABI = async (req, res) => {
    try {
